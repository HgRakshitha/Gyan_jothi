...
/// Home coloring — same interaction as [ColorTheTreePage].
class ColorMyHomePage extends StatefulWidget {
  const ColorMyHomePage({super.key});

  @override
  State<ColorMyHomePage> createState() => _ColorMyHomePageState();
}
...
class _ColorMyHomePageState extends State<ColorMyHomePage> {
  ...
  static const Color _graySwatch = Color(0xFFA9A9A9);
  static const Color _skySwatch = Color(0xFFA5DFF5);
  ...
  static const List<_PaletteEntry> _palette = <_PaletteEntry>[
    ...
    _PaletteEntry(label: 'Sky', color: _skySwatch),
  ];
  ...
  void initState() {
    super.initState();
    _selectedColor = _brownSwatch;
    _loadMaskImage();
  }

  Future<void> _loadMaskImage() async {
    try {
      final data = await DefaultAssetBundle.of(context).load(AppAssets.artsCraftColorMyHomeLineArt);
  ...
  Future<void> _shareCanvas() async {
    ...
    try {
      ...
      final files = await buildPngShareFiles(bytes, 'color_my_home.png');
                              ...
                              Padding(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      'Choose a color below and paint the house',
                                      textAlign: TextAlign.center,
                                        ...
                                        return RepaintBoundary(
                                          ...
                                          child: SizedBox(
                                            ...
                                            child: _maskImage == null
                                                ? AppAssetImage(
  assetPath: AppAssets.artsCraftColorMyHomeLineArt,
  width: w,
  height: h,
  fit: BoxFit.contain,
  fallback: Icon(
...
class _ColorMyHomeDoneButton extends StatelessWidget {
  const _ColorMyHomeDoneButton({required this.onPressed});
...
class _HomePaintOverlay extends CustomPainter {
  _HomePaintOverlay({
  ...
  @override
  bool shouldRepaint(covariant _HomePaintOverlay oldDelegate) {
    return true;
  }
}
