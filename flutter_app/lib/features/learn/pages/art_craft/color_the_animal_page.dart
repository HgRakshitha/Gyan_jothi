...
/// Animal coloring — same layout as [ColorTheTreePage].
class ColorTheAnimalPage extends StatefulWidget {
  const ColorTheAnimalPage({super.key});

  @override
  State<ColorTheAnimalPage> createState() => _ColorTheAnimalPageState();
}
...
class _ColorTheAnimalPageState extends State<ColorTheAnimalPage> {
  static const Color _pageBg = Color(0xFFFFFFFF);
  static const Color _cardBg = Color(0xFFFBFBE4);
  static const Color _graySwatch = Color(0xFF9E9E9E);
  static const Color _blackSwatch = Color(0xFF212121);

  static const List<_PaletteEntry> _palette = <_PaletteEntry>[
    _PaletteEntry(label: 'Gray', color: _graySwatch),
    _PaletteEntry(label: 'Black', color: _blackSwatch),
  ];
  ...
  void initState() {
    super.initState();
    _selectedColor = _graySwatch;
    _loadMaskImage();
  }

  Future<void> _loadMaskImage() async {
    try {
      final data = await DefaultAssetBundle.of(context).load(AppAssets.artsCraftColorAnimal);
  ...
  Future<void> _shareCanvas() async {
    ...
    try {
      ...
      final files = await buildPngShareFiles(bytes, 'color_the_animal.png');
                              ...
                              Padding(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      'Choose a color below and fill in the elephant',
                                      textAlign: TextAlign.center,
...
class _ColorAnimalDoneButton extends StatelessWidget {
  const _ColorAnimalDoneButton({required this.onPressed});
...
class _AnimalPaintOverlay extends CustomPainter {
  _AnimalPaintOverlay({
  ...
  @override
  bool shouldRepaint(covariant _AnimalPaintOverlay oldDelegate) {
    return true;
  }
}
