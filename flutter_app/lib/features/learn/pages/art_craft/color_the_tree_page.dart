...
/// Palette colors for tree page swatches.
const Color _kColorTreeBrown = Color(0xFFBC8E7F);
const Color _kColorTreeGreen = Color(0xFFC5E083);

/// Line-art tree coloring activity — matches "Color the Tree" UI spec.
class ColorTheTreePage extends StatefulWidget {
  const ColorTheTreePage({super.key});

  @override
  State<ColorTheTreePage> createState() => _ColorTheTreePageState();
}
...
class _ColorTheTreePageState extends State<ColorTheTreePage> {
  ...
  static const Color _brownSwatch = _kColorTreeBrown;
  static const Color _greenSwatch = _kColorTreeGreen;
  ...
  void initState() {
    super.initState();
    _selectedColor = _greenSwatch;
    _loadMaskImage();
  }

  Future<void> _loadMaskImage() async {
    try {
      final data = await DefaultAssetBundle.of(context).load(AppAssets.artsCraftColorTheTree);
  ...
  Future<void> _shareCanvas() async {
    ...
    try {
      ...
      final files = await buildPngShareFiles(bytes, 'color_the_tree.png');
                              ...
                              Padding(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      'Choose a color below and fill in the tree',
                                      textAlign: TextAlign.center,
                                        ...
                                        return RepaintBoundary(
                                          ...
                                          child: SizedBox(
                                            ...
                                            child: _maskImage == null
                                                ? AppAssetImage(
  assetPath: AppAssets.artsCraftColorTheTree,
  width: w,
  height: h,
  fit: BoxFit.contain,
  fallback: Icon(
...
class _ColorTreeDoneButton extends StatelessWidget {
  const _ColorTreeDoneButton({required this.onPressed});
...
/// Brush strokes on top of the tree image (full white card; [BoxFit.cover]).
class _TreePaintOverlay extends CustomPainter {
  _TreePaintOverlay({
  ...
  @override
  bool shouldRepaint(covariant _TreePaintOverlay oldDelegate) {
    return true;
  }
}
